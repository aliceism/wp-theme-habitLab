...
?>
<section id="insights-home" class="insights-home section" aria-labelledby="insights-home-title">
    <div class="container insights-home__inner">
        <div class="insights-home__copy">
            <p class="insights-kicker"><?php esc_html_e('HabitLab Insights', 'habitlab'); ?></p>
            <h1 id="insights-home-title" class="insights-home__title"><?php esc_html_e('Build Better Habits, One Day at a Time', 'habitlab'); ?></h1>
            <p class="insights-home__lead"><?php esc_html_e('Practical articles on discipline, motivation, and behavior change for real daily consistency.', 'habitlab'); ?></p>
            <div class="insights-home__actions">
                <a class="btn btn-primary" href="#insights-latest"><?php esc_html_e('Start Reading', 'habitlab'); ?></a>
                <a class="btn btn-ghost" href="<?php echo esc_url($habitlab_blog_url); ?>"><?php esc_html_e('View All Articles', 'habitlab'); ?></a>
            </div>
        </div>
        <aside class="insights-home__panel" aria-label="<?php esc_attr_e('What you will learn', 'habitlab'); ?>">
...
<?php if ($habitlab_featured_post instanceof WP_Post) : ?>
    ...
    $habitlab_featured_read_time = function_exists('habitlab_get_post_read_time')
        ...
        : 1;
    ?>
    <section class="insights-featured section" aria-labelledby="insights-featured-title">
        <div class="container">
            <header class="insights-section-head">
                <p class="insights-kicker"><?php esc_html_e('Featured', 'habitlab'); ?></p>
                <h2 id="insights-featured-title"><?php esc_html_e('Start With This', 'habitlab'); ?></h2>
            </header>

            <article class="insights-featured__card card card--hover">
                <a class="insights-featured__media" href="<?php echo esc_url((string) $habitlab_featured_permalink); ?>">
                    <?php if (has_post_thumbnail($habitlab_featured_id)) : ?>
                        ...
                        ?>
                    <?php else : ?>
                        <span class="insights-thumb-fallback"><?php esc_html_e('Featured Article', 'habitlab'); ?></span>
                    <?php endif; ?>
                </a>
                <div class="insights-featured__content">
                    <p class="insights-featured__meta">
...
<?php if (is_array($habitlab_topic_categories) && $habitlab_topic_categories !== []) : ?>
    <section class="insights-topics section" aria-labelledby="insights-topics-title">
        <div class="container">
            <header class="insights-section-head">
                <p class="insights-kicker"><?php esc_html_e('Topics', 'habitlab'); ?></p>
                <h2 id="insights-topics-title"><?php esc_html_e('Explore By Category', 'habitlab'); ?></h2>
            </header>

            <div class="insights-topics__grid">
                <?php foreach ($habitlab_topic_categories as $habitlab_index => $habitlab_term) : ?>
                    ...
                    $habitlab_accent_class = 'insights-topic-card--' . (($habitlab_index % 6) + 1);
                    ?>
                    <a class="insights-topic-card card card--hover <?php echo esc_attr($habitlab_accent_class); ?>" href="<?php echo esc_url((string) $habitlab_term_link); ?>">
                        <span class="insights-topic-card__name"><?php echo esc_html($habitlab_term->name); ?></span>
                        <span class="insights-topic-card__count">
                            <?php
                            printf(
...
<section id="insights-latest" class="insights-latest section" aria-labelledby="insights-latest-title">
    <div class="container">
        <header class="insights-section-head">
            <p class="insights-kicker"><?php esc_html_e('Latest Articles', 'habitlab'); ?></p>
            <h2 id="insights-latest-title"><?php esc_html_e('Fresh Insights For Better Consistency', 'habitlab'); ?></h2>
        </header>
        ...
        <?php if (is_array($habitlab_latest_posts) && $habitlab_latest_posts !== []) : ?>
            <div class="insights-post-grid">
                <?php foreach ($habitlab_latest_posts as $habitlab_post) : ?>
                    ...
                    ?>
                    <article class="insights-post-card card card--hover">
                        <a class="insights-post-card__media" href="<?php echo esc_url((string) $habitlab_permalink); ?>">
                            <?php if (has_post_thumbnail($habitlab_post_id)) : ?>
                                ...
                                ?>
                            <?php else : ?>
                                <span class="insights-thumb-fallback"><?php esc_html_e('HabitLab', 'habitlab'); ?></span>
                            <?php endif; ?>
                        </a>
                        <div class="insights-post-card__body">
                            <p class="insights-post-card__meta">
        ...
        <?php else : ?>
            <p class="insights-empty"><?php esc_html_e('No articles published yet. Add your first post to start the journal.', 'habitlab'); ?></p>
        <?php endif; ?>
    </div>
</section>

<?php if (is_array($habitlab_guide_posts) && $habitlab_guide_posts !== []) : ?>
    <section class="insights-guides section" aria-labelledby="insights-guides-title">
        <div class="container">
            <header class="insights-section-head">
                <p class="insights-kicker"><?php esc_html_e('Practical Guides', 'habitlab'); ?></p>
                <h2 id="insights-guides-title"><?php esc_html_e('Actionable Frameworks You Can Use Today', 'habitlab'); ?></h2>
            </header>

            <div class="insights-guides__grid">
                <?php foreach ($habitlab_guide_posts as $habitlab_post) : ?>
                    ...
                    $habitlab_permalink = get_permalink($habitlab_post_id);
                    ?>
                    <article class="insights-guide-card card card--hover">
                        <p class="insights-guide-card__meta"><?php echo esc_html(function_exists('habitlab_get_primary_category_label') ? habitlab_get_primary_category_label($habitlab_post_id) : __('Article', 'habitlab')); ?></p>
                        <h3><a href="<?php echo esc_url((string) $habitlab_permalink); ?>"><?php echo esc_html(get_the_title($habitlab_post_id)); ?></a></h3>
                        <p><?php echo esc_html(wp_trim_words((string) get_the_excerpt($habitlab_post_id), 18)); ?></p>
                        <a class="insights-guide-card__link" href="<?php echo esc_url((string) $habitlab_permalink); ?>">
